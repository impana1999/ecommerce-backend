 ...
 */
export const isEmpty = (value: string | number | boolean | Array<unknown> | object): boolean => {
  if (value === null) {
    return true;
    ...
    return true;
  } else if (value !== null && typeof value === 'object' && Array.isArray(value) && !value.length) {
    return true;
  } else {
    return false;
  }
...
export const customReplaceAll = (input: string, find: string, replace: string): string => {
...
export const removeBlankValue = (value: any) => {
  if (value !== null && typeof value === 'object' && !Array.isArray(value) && !Object.keys(value).length) {
    for (const i in value) if (value[i] === '' || value[i] === undefined) delete value[i];
